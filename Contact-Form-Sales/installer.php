 ...
 * Version: 1.2 - FULLY WORKING VERSION
 *
...
if ($get_action === 'install' || $post_action === 'install') {
    ...
    // Security check
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !hash_equals(isset($_POST['csrf_token']) ? $_POST['csrf_token'] : '', isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '')) {
        echo json_encode(['success' => false, 'error' => 'CSRF token mismatch']);
        exit;
    }

    try {
        $get_step = isset($_GET['step']) ? $_GET['step'] : ''; $post_step = isset($_POST['step']) ? $_POST['step'] : ''; $step = $get_step ?: $post_step ?: 'check';
